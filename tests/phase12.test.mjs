...
test('market is the highest-weight competitor signal',()=>{assert.match(scoring,/marketOverlap:\.25/);assert.match(scoring,/currencyMatch:\.10/);assert.match(scoring,/languageMatch:\.10/)});
test('known market mismatch applies a hard score cap',()=>{assert.match(scoring,/differentKnownMarket/);assert.match(scoring,/Math\.min\(match,55\)/)});
