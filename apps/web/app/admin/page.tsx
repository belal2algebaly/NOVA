...
export default async function AdminConsole(){
 ...
 const active7=users.filter((u:any)=>u.last_sign_in_at&&now-new Date(u.last_sign_in_at).getTime()<=7*DAY).length;
 ...
 return <main className="shell"><Sidebar/><section className="workspace adminWorkspace">
  <header className="pageHeader adminHero"><div><p className="eyebrow">PLATFORM CONTROL</p><h1>Super Admin</h1><p className="muted">Live visibility across NOVA users, workspaces and product activity</p></div><span className="superBadge">♛ BELAL · ROOT ACCESS</span></header>
  ...
  <article className="panel adminUsersPanel"><div className="panelhead"><div><p className="eyebrow">USER DIRECTORY</p><h2>Registered users</h2><p className="muted compactCopy">Account status, activity and ownership at a glance</p></div><span className="pill">{users.length} accounts</span></div>
