...
export default async function Signup({searchParams}:{searchParams:Promise<{error?:string}>}){
 ...
 return <main className="authShell"><section className="authCard"><Brand/><div className="authHeading"><p className="eyebrow">CREATE WORKSPACE</p><h1>Start with NOVA</h1><p>Your stores, audits and competitors live in one account</p></div>{!isSupabaseConfigured&&<SetupNotice/>}{q.error&&<div className="alert error">{q.error}</div>}
 <form action={signInWithGoogle}><ActionButton className="googleButton wide" pendingLabel="Opening Google…"><GoogleIcon/><span>Sign up with Google</span></ActionButton></form>
 <div className="authDivider"><span>or create with email</span></div>
 <form action={signUp} className="form"><label>Name<input name="name" required autoComplete="name"/></label><label>Email<input name="email" type="email" required autoComplete="email"/></label><label>Password<input name="password" type="password" minLength={6} required autoComplete="new-password"/><small>Minimum 6 characters</small></label><ActionButton className="primary wide" pendingLabel="Creating account…">Create account</ActionButton></form>
 <p className="activationHint">We’ll send an activation link to your email. No manual approval is required</p>
 <p className="authFoot">Already have an account? <Link href="/login">Sign in</Link></p></section></main>
}
