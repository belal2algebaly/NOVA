...
export default async function Login({searchParams}:{searchParams:Promise<{error?:string,message?:string}>}){
 ...
 <form action={signInWithGoogle}><ActionButton className="googleButton wide" pendingLabel="Opening Google…"><GoogleIcon/><span>Continue with Google</span></ActionButton></form>
 <div className="authDivider"><span>or continue with email</span></div>
 <form action={signIn} className="form"><label>Email<input name="email" type="email" required autoComplete="email"/></label><label>Password<input name="password" type="password" required autoComplete="current-password"/></label><ActionButton className="primary wide" pendingLabel="Signing in…">Sign in</ActionButton></form>
