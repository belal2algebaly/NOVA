...
export async function aiReviewCompetitorEvidence(own:StoreProfile,candidate:StoreProfile,scored:any):Promise<{verdict:'confirm'|'caution'|'reject';confidence:number;reason:string;provider?:string}|null>{
 const system=`You are NOVA's second-pass direct-competitor verifier. Review ONLY supplied crawl/scoring evidence. You may CONFIRM, CAUTION, or REJECT an automatically accepted candidate. You must NEVER promote a candidate that failed deterministic gates. Reject only for a clear contradiction such as wrong market, no real product overlap, marketplace/directory behavior, or incompatible commerce context. Return JSON only.`;
 const result=await providerJson(system,`OWN STORE:\n${JSON.stringify(context(own),null,2)}\n\nCANDIDATE:\n${JSON.stringify(context(candidate),null,2)}\n\nDETERMINISTIC SCORE:\n${JSON.stringify({classification:scored.classification,match:scored.match,confidence:scored.confidence,signals:scored.signals,evidence:scored.evidence,rejectionReasons:scored.rejectionReasons},null,2)}\n\nReturn {"verdict":"confirm|caution|reject","confidence":0-100,"reason":"one concise evidence-based sentence"}.`);
 if(!result)return null;const v=String(result.data?.verdict||'caution').toLowerCase();return {verdict:v==='confirm'||v==='reject'?v:'caution',confidence:clamp(Number(result.data?.confidence||0)),reason:String(result.data?.reason||'AI evidence review completed'),provider:result.provider};
}
